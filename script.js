...
document.addEventListener('DOMContentLoaded', function(){
  ...
  const render = () => {
    ...
    const avatarImages = data.profile.avatarImages || [data.profile.avatarImage];
    const avatarCards = avatarImages.map((image, index) => `<div class="avatar" tabindex="0" role="img" aria-label="${data.profile.avatarLabel} ${index + 1}"><img class="avatar-photo" src="${image}" alt="${data.profile.avatarLabel} ${index + 1}"><span class="avatar-hint" aria-hidden="true">MOVE TO REVEAL</span></div>`).join('');
    document.querySelector('[data-module="hero"]').innerHTML = `
      <div class="hero-left"><span class="hero-kicker">PERSONAL PROFILE / 2026</span><h1>${data.profile.headline}</h1><p class="lead">${data.profile.role} · ${data.profile.introduction}</p><p class="quick">${data.profile.location}</p><div class="hero-cta"><a class="btn primary" href="#projects">查看作品</a><a class="btn ghost" href="#contact">联系我们</a></div></div><div class="hero-right"><div class="avatar-gallery">${avatarCards}</div></div>`;
